...
// memory (-y/--yes -> --defaults, init only); redisctl owns everything else.
// Exit codes forward verbatim (0 success / 2 usage / 6 validation / 10 network
// / 12 cancelled).
'use strict';
...
const args =
  rawArgs[0] === 'init'
    ? rawArgs.map((arg, i) =>
        i > 0 && (arg === '-y' || arg === '--yes') ? '--defaults' : arg
      )
    : rawArgs;
