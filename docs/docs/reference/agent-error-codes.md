...
| `account_required` | 2 | no | An account id is needed and cannot be chosen: `cloud auth switch` with no terminal to ask on, or a `cloud auth login` whose sign-in does not report which of several accounts is current. Pass the id with `--account`; a completed login lists the accounts you belong to in its `accounts` field. |
