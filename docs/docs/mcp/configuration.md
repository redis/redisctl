...
!!! note "Where credentials may be written"
    ...
    project — which is why the directory list exists. The CLI's
    `cloud workflow quick-database` is unaffected: it takes its path from whoever is at the
    terminal, not from a model.
